...
window.geometry("780x670+200+70")
bomb_number=40  #炸彈
game_Y_size=16   #高度
game_X_size=16   #寬度
btn=[]          #按鈕
...
'''=========================<新遊戲設定>============================'''
def new_play_game():
    Label(window, textvariable=remain_bomb,font='160',height='3',bg='#d0d0d0').place(y = 10,x = 2*game_X_size)
    Label(window, textvariable=labelText,font='160',height='3',bg='#d0d0d0').place(y = 10,x = 44*game_X_size-1)
    smile=Button(window,textvariable=change_smile,font='200',height='2',bg='Snow')
    smile.bind("<Button-1>",smile_do)#左鍵
    smile.place(y = 15,x = int(22*game_X_size+10/2))
new_play_game()
'''=========================<左右鍵搜尋>============================'''
...
def set_button():
    ...
    for y in range(game_Y_size):
        btn.append([])
        for x in range(game_X_size):
            btn[y].append(Button(window,width = 3,height = 1, relief='raised',anchor='center'))
            ...
            btn[y][x].config(text=' ',font='15', bg='#c6c6d3',fg=text_color[chess[y][x]])
            btn[y][x].place(y = 34*y+80,x = 44*x+20)
set_button()
window.mainloop()