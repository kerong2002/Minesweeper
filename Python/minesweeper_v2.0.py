...
window.geometry("340x450+600+200")
bomb_number=10  #炸彈
game_Y_size=9   #高度
game_X_size=9   #寬度
btn=[]          #按鈕
...
def normal_mod():
    ...
    time_text.place_forget()
    global game_X_size,game_Y_size,bomb_number,last_game
    window.geometry("580x670+500+100")
    game_X_size = 16
    game_Y_size = 16
    bomb_number = 40
    smile_do(True)
...
menuFile.add_command(label='easy',font=('Inconsolata',10), command=easy_mod)#難度
menuFile.add_command(label='normal',font=('Inconsolata',10), command=normal_mod)#難度
...
def set_button():
    ...
    for y in range(game_Y_size):
        ...
        for x in range(game_X_size):
            ...
            btn[y][x].place(y = 34*y+80,x =x*34+20)
set_button()
window.mainloop()
